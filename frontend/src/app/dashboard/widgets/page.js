...
export default function WidgetsPage() {
        ...
        try {
            ...
            const response = await axios.post(
                `${API_URL}/chat`,
                {
                    ...
                    message: `Generate a concise system prompt (max 400 characters) for an AI assistant that helps with: ${promptTopic}. Only output the system prompt text, nothing else.`
                },
                { headers: { Authorization: `Bearer ${token}` } }
            );

            if (response.data.success && response.data.message) {
                const generatedPrompt = response.data.message.substring(0, 500);
    ...
    return (
        <div className="p-4 md:p-8 min-h-screen lg:h-[calc(100vh-64px)] overflow-hidden flex flex-col">
            {/* Header */}
            <div className="mb-6 flex-shrink-0">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Widget Configuration</h1>
                <p className="text-gray-600 dark:text-gray-300 mt-1">Customize appearance, AI behavior, and install your chat widget</p>
            </div>

            {/* Tabs */}
            <div className="flex gap-2 sm:gap-4 border-b border-gray-200 dark:border-gray-700 mb-6 flex-shrink-0 overflow-x-auto pb-px">
                {[
                    ...
                    <button
                        ...
                        className={`pb-3 px-2 sm:px-3 font-medium text-sm transition-all relative flex items-center gap-1.5 whitespace-nowrap ${activeTab === tab.id
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`text-xs ${widgetSettings.systemPrompt.length > 450 ? 'text-red-500' : 'text-gray-500'}`}>
                                        {widgetSettings.systemPrompt.length}/500
                                    </span>
                                </div>
                                <textarea
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            <div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-2">
                                    <div className="flex-1 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 font-mono text-sm relative group overflow-hidden">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="flex-1 sm:flex-none p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center justify-center"
                                        >
